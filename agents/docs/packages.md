...
- Convenience constructors → `*Builder` (patterns.go): `Email()`, `URI()`, `UUID()`, `Date()`, `DateTime()`, `NonEmptyString()`, `PositiveNumber()`, `PositiveInteger()`, `Enum(...any)`, `OneOf(...*Schema)`, `AnyOf(...*Schema)`, `AllOf(...*Schema)`, `Optional(*Schema)` (schema-or-null).
